...
.stTextInput input:focus {
    ...
    border-color: #4f46e5 !important;  /* 보라계열 */
...
# ==============================
# 검색 UI (폼 기반: Enter/버튼 검색)
# ==============================
DEFAULTS = {
    "query": "",
    "mode_idx": 0,  # 0: AND, 1: OR
...
with st.form("search_form", clear_on_submit=False):
    c1, c2 = st.columns([1, 0.32])  # 입력창 크게, 버튼은 좁게
    with c1:
        query_in = st.text_input(
            "검색어 입력 (쉼표로 여러 개, 예: 소독기, 필름, 펌프)",
            value=st.session_state["query"],
            placeholder="예) 트랙터, 살포기, 분무기"
        )
    with c2:
        submitted = st.form_submit_button("검색(Enter)")

    mode_labels = ["AND", "OR"]
    mode_and_or = st.radio("검색 방식", mode_labels, index=st.session_state["mode_idx"], horizontal=True)
    case_sensitive = st.toggle("대소문자 구분", value=st.session_state["case_sensitive"])

    # 제출되면 상태 업데이트
    if submitted:
        st.session_state["query"] = query_in
        st.session_state["mode_idx"] = mode_labels.index(mode_and_or)
        st.session_state["case_sensitive"] = case_sensitive

# 상태값으로 실제 검색 실행 (페이지 재실행 시에도 동일 결과 유지)
active_query = st.session_state["query"]
active_mode = ["AND", "OR"][st.session_state["mode_idx"]]
active_case = st.session_state["case_sensitive"]

results = search(df, active_query, case_sensitive=active_case, mode=active_mode)
